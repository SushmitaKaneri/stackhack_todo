...
class Bin extends Component {
  ...
  handle_delete(e){
    ...
    axios.post(server_url+'/api/upload/deletesection',{_id:e.target.id})
    .then((result)=>{
...
pin(e){    
  ...
  loadNotes(){
    axios.post(server_url+'/api/upload/search', {email:this.state.user,status:'Bin'})
      .then((result) => {
  ...
  Section(){
    ...
    <Col xs="12" sm="6" md="3">
        ...
        <CardHeader style={{color:"#61c0bf",background:"#ecfbfc"}}> 
          ...
          <Button color="primary" id={this.state.result==null?"":e._id} onClick={this.handle_delete} style={{'float':'right'}}>Delete</Button>
        </CardHeader>
